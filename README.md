...
**Update**: I can get neither Docker nor Vagrant to install on my build :( The errors are plentiful and seem to change contantly.  ~This issue has me completely re-thinking the suitability of WSL for my particular use case :( Since i can't actually upgrade my build, I believe I may actually need to consider going back to Cygwin~ (which is just incredibly disappointing...). Ok, that reaction was a bit overwrought in hindsight...While I'm still disappointed that this can't unfortunately work for me, it's less of a WSL "deal-breaker" than I had been winding myself up over, and in my case Babun/Cygwin wouldn't appear to solve the problem either :(
