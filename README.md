...
If your search for guidance on setting up a reasonable dev environment on WSL has lead you here, I hope that through offering this fairly opinionated, sometimes rambling, occassionally solipsistic, yet well-intentioned sharing of collected observations, example configurations/dotfiles, and earnest efforts to honestly capture and adequately journal my various experiences interacting with Microsoft's Windows Subsystem for Linux (WSL) available as an optionally enabled feature of the Windows 10 operating system.

I also leverage the deployable ease of Homesick in distributing and managing my configuration files, which I've also included here as an example Homesick "castle" which you are free to clone/download, and is intended as a meaningful way to expand the descriptive dimensions of the discussion that follows. However, the usual "ymmv", "No warranty either express or implied", "Use of this repo may totally break your sh\*t" - rules governing personal responsibility most definitely apply here, and you should certainly always be weighing the associated risks of hastily cloning some random dotfiles project into your environment off Github (or really in introducing any new code obtained from anywhere on the Internet) :)
...
The friction from what is currently available would even seem to persist as abrasions to collaborative productivity well into a project's term and varying in severity based on project character or requirements (i.e. Golang projects would seem to meet with much less initial resistance when developed on other platforms...), and by incremental extension likely to chafe any credible "bottom-up" effort at process-engineering as related to a cohesive idea of DevOps/CI/CD or serious initiative aimed at "Digital Transformation".
