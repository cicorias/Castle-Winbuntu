...
Since I believe I've now spent enough time experimenting with WSL to form a reasonable credible opinion - perhaps even earning the right to express this editorially - and given how crucial I feel this is to minimizing any substantial impact to personal productivity/expected rate of contribution when transitioning to a Windows-based workflow, *I'll just come right out and say that:* **There are currently no [terminal applications for Windows](https://raw.githubusercontent.com/rodtreweek/i/master/ansible/term_probs.gif) that compare favorably to those offered natively for Linux or to the (excellent) iTerm2 for Mac**.

The friction resulting from such exclusivity abrasively persisting throughout a project's term, variously corrosive depending on the specific character or requirements for a particular project (i.e. golang/node.js projects would seem to benefit less from an exclusively Windows-native development workflow...), and by incremental extension likely to chafe any credible "bottom-up" process-engineering of CI/CD, or any serious initiatives aimed at "Digital Transformation".
