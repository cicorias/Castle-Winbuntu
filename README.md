...
**An additional word of warning here**...You may also find (as I have just today...) that after spending an hour troubleshooting an SSH key-based authentication issue, where there might be **no discernible whitespace** in the authorized_keys file, yet you're still getting password-prompted even after *quadruple-checking all permissions* (~btw, reminder on permissions: `700` on `.ssh`, and `600` on `authorized_keys`...OHH!, and *700 on your home directory as well!*~)and *painstakingly* examining the output of `ssh user@host -vvvvvvvvvvvvvvvvv` for clues.  **This is likely due to the fact that you or an end-user probably copy/pasted the contents of id_rsa.pub file into/from Outlook** (perhaps even errantly clipping a couple characters in the process...), or Outlook has somehow done some form of "smart" formatting on it, *invisibly* changing some of the characters or otherwise introducing some other extraneous formatting. Unless you want to go through the unnecessary motions of checking every link in this particular chain, I'd recommend you *insist* (gently, of course) that any public keys you intend on placing on a host arrive in a "tamper-resistant" `.pub` file, which you can then use something like the unix `ssh-copy-id` utility to add to a host's authorized_keys file, **Oh, also ssh-dss keys don't work anymore either post-OpenSSh 7.0** - which sort of passed me by I guess...I'm still trying to hunt down sufficient detail as to this decision, and so far have only really found this: https://meyering.net/nuke-your-DSA-keys/ - which was written 7 years ago in 2010, and incidentally a full five years prior to the release of OpenSSH 7.0 where support for DSA was initially dropped :(
