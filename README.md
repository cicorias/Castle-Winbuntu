<img src="https://raw.githubusercontent.com/rodtreweek/i/master/castle-winbuntu/castle_winbuntu_welcome.png" height="450">
...
While upgrading was perhaps the right decision for several other reasons, it really didn't resolve either of the issues I'd sought to remedy initially; the YCM plugin still complained about the version of Vim, and `sudo apt-get install` now seemed even more busted than ever, having now added a merely decorative new `tty error` to it's limited output. I finally resolved this by downgrading several packages (--see the `wget` and `dpkg -i` and `/dev/tty` permissions changes noted above...) however each time I was subsequently tempted to run `sudo apt-get update`, I reflexively hesitated, wondering if this might upset the seemingly fragile balance I'd worked to achieve. Again, in retrospect these issues were highly build/environment specific, (and in retrospect perhaps due in part to my misunderstanding of how sudo/root actually works in WSL), so would seem comparitively rare. Again, YMMV. Since this was a much less straightforward and time-consuming process than I had expected, I'm including this information here just in case it's helpful to others :)
