...
I've since understood that 16.04 is technically unsupported on pre-"Creator" builds, and should be aware that upgrading *may invite a number of other unexpected "guests" to the party* - which may not be quite so desirable depending on your reasons for upgrading, etc. The point here is that you should be clear on what problem you are trying to solve, and reasonably clear on how this would be solved by upgrading to 16.04. Again, my decision to upgrade was prompted mainly by a pair of *really* frustrating issues that I *thought* would be resolved by upgrading to Ubuntu 16.04:
