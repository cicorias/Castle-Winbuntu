...
1. In the "Startup directory" field, enter: `C:\Users\<your user>`
...
Before switching to [MobaXterm](https://mobaxterm.mobatek.net/) I had previously been using [ConEmu](https://conemu.github.io) as my terminal application, as it felt (to me) the most similiar to iTerm2 - a Mac-specific mainstay that had previously been at the center of my workflow.  Having tried Hyper.js, wsltty and a few others (the names of which I'm now forgetting), ConEmu - while not quite the perfect replacement for iTerm2 I had hoped for - easily emerged as the most stable, configurable, and fully-featured of those I had tried. It also offered tabbed-sessions, ~which I can't really live without~ which up to that point had been a firm requirement for me, even as I warmed slightly to the exclusive use of tmux to boundary my sessions. Indeed, ConEmu has done much to earn its considerable popularity; it is arguably the best terminal solution for maintaining maximum interoperability with Windows, uniquely ambitious in consolidating utilities for device configuration, maintenance, and remote management tasks while matched with perhaps the most comprehensive range of configuration options of any terminal solution currently available - and unlikely to disappoint anyone capable of leveraging a majority of its features - i.e. those less likely to exceed the 3 minute - let alone 3 hour mark in discovering that "run as an Administrator" is surprisingly effective at getting `ping` to return something other than "Permission denied" :(  After what was marginally less painful than assisting my father over the phone in searching his Gmail account for a 6 year old email attachment from an unknown sender, I eventually realized that I also needed to open up Settings --> Startup --> Tasks, and add the `-new_console:a` flag to my executable string  so that it looked like this `%windir%\system32\bash.exe ~ -new_console:a` . 

<img src="https://raw.githubusercontent.com/rodtreweek/i/master/castle-winbuntu/conemu_bash_admin.png" height="450">

After clicking `Save settings` Bash no longer complained about permissions/privileges.

FWIW, I'm now recalling more completely why tabbed-sessions had previously held such appeal for me. While a local tmux config is great for maintaining *local* session boundaries, even if you are careful to detach from them, they are still tethered to a *local* process, -- meaning that the now-in-progress compile of Vim 8 you just kicked-off on a remote host isn't going to withstand the incoming forced-reboot naturally triggered by Windows Update minutes later ;) 

To avoid this potential calamity you *still* need some mechanism by which to automatically start/create or re-attach to existing sessions on remote hosts (and subsequently boundary these sessions).  This is where tabbed-sessions is particulary useful, since each remote connection can then be represented in it's own tab - and (to me) much more intuitively mapped in a 1-to-1 manner to its own individual tmux session - initiated and maintained on the *remote* host.

In the past, since it was common that I might be connected to several remote systems at any given time (typically using `sudo -i` or `sudo su -`), it was extremely useful (I might even say it was a requirement...) for me to maintain several tabbed-sessions each of which was bound to some conditional logic in both my local and remote tmux.conf files and aliased to the `autossh` command to establish what was essentially a "Close the lid on laptop. Drive home. Open lid on laptop. Trigger vpn connection - now wait for sessions to be restored in each tab..." - method of persistant connectivity. While this sort of "sticky" ssh connectivity now feels like an anachronism particularly in lieu of "Infrastructure as Code" and the need to balance this against any potential security implications (for example, at a minimum great care should be taken in preventing ssh-agent process-sprawl), at the time it was pretty hard not to appreciate the friction-free convenience of this configuration  - especially at 3am when the increasing frequency of CRITICAL alerts converged to form a singular, uninterrupted tone - announcing the arrival of a 48-hour-work-marathon ;)
...
In retrospect these issues were again highly build/environment specific, (and I'll admit due ~in part~ to my misunderstanding of ~how sudo/su actually works with environment variables in WSL~ *how sudo really just works in general* - see my table [below](#another-quick-note-on-sudo)), and would seem comparatively rare, especially for those who have already carefully checked their build version for any surprises and are on anything other than the earliest Windows 10 builds ;) In any case, since this was a much less straightforward and time-consuming process than I had anticipated, I'm including this information in case it might be useful to the similarly impetuous :)

## TL;DR - The "Meat and Potatoes" of deploying the dotfiles...

## Acknowledgements!

I'd also like to send a super-appreciative shout-out to all those who so generously share their time and effort on Github assisting others in building and shaping rapidly deployable configurations. The bulk of my dotfiles are really just a curation of extremely useful things I've either lightly iterated on, -- or simply lifted outright from others (with permission of course). Several were sourced initially from Jessie Frazelle, - who through her work with Docker, Google (and now Microsoft!), continues to impressively shape many notable innovations while promoting FOSS/OSS and remaining unfailingly generous in offering considerable guidance on a range of engineering/development issues, -- and also Nate Mccurdy from Puppet, who - in addition to providing the principal inspiration for this repo - continues to generously offer his elegantly tailored, thoughtfully maintained and rigorously "customer-prem battle-tested" code/configuration for a rapidly deployable Ruby/Puppet development workflow --- You constantly inspire me through your intelligence and generosity --- Thank you both!

Ok, I hear ya...sounds like you're hungry. Here's how to deploy:

## Shell
I've been a pretty big fan of [oh-my-zsh](https://github.com/robbyrussell/oh-my-zsh), and have been using it ~exclusively~ on WSL for some time now (previously I had tried bash-it, which also worked ok - if a bit more sluggishly...)  **Update:** Despite its modest performance improvements over bash-it - even when frugally configured - OMZ still performs less capably in WSL than on my Mac :( This disparity also does not appear diminished at any point along the path from pre-"Creator" to recent 17.x builds :( In fact, simply adding pyenv and configuring a handful of options for Powerlevel9k on my 17.x build - when "weighed" with ` zsh -xv` - reveals just how heavily this can inhibit performance...which has lead to some pretty disappointing (and often only marginally effective) tradeoffs. ~**I've now switched to [Prezto](https://github.com/sorin-ionescu/prezto) - which has been performing *significantly better* in just about every way.** Not only has it performed at easily twice the speed of OMZ, it is also *much* simpler to configure and customize.~  Well that didn't take long. Despite a strong start, Windows Defender has now caught up with Prezto as well - rendering it just as slow :( https://github.com/Microsoft/WSL/issues/1932
### Bash
...
5. **Note:** If after installing zplug and configuring your environment you begin seeing various directory "insecure" messages pop-up in WSL --execute the following: `compaudit | xargs sudo chmod -R go-w` along with: `sudo chmod 755 /usr/local/bin /tmp` to remove group/owner write permissions on these directories.
...
1. ~Setting up YouCompleteMe natively to support code-completion in WSL is unfortunately still a WIP for me, and~ I'd been largely pretty blown away by YCM when I first encountered it a couple of years back, and it still remains one of my favorite Vim plugins - when it can be made to work correctly/predictably. The lengthy list of dependencies and fairly complicated sequence of installation/configuration steps often idiosyncratic for whichever programming language(s) you may need it to support, already long and arduous on a Mac, was a significantly lengthier and more perplexing ordeal on WSL - and ultimately a failed experiment for me:( Although this again appears to be mainly due to the limitations of my Windows/WSL build, the dated versions of Vim packaged with both Ubuntu Trusty and Xenial certainly haven't helped matters, nor am I able to recommend any of the sparse resources I found that basically gave me false-hope in the end.). 
...
* *Upgrade to Vim 8* (I haven't used Neovim, which might be an option as well...I've spent quite a lot of time on my configs, so unless these are "plug 'n' play" with Neovim, I'm a bit reticent to experiment...). There are quite a lot of new features in this version, and if you intend on using the `.vimrc` and `.vimrc.settings` files included in this repo then **Vim version 8.x is required**. Although it's a bit of a pain to build this from source, if you are serious about using Vim as more or less your IDE on WSL, then I think it's also a bit worthwhile to get an idea as to what's contained in the source (or if you don't have the time just go [here](https://gist.github.com/rodtreweek/894f02a23bbc7e3691fa1a0f954e3a40) :)
