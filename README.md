...
The friction resulting from such a limited number of comparatively clumsy/imprecise terminal emulation options can be observed to persist abrasively over the course of a project's term, resulting in the corrosive atrophy of collaborative speed and diminished iterative improvements, and by which this pipeline-damaging potency can typically be measured relative to the composition and complexity of the requirements (for instance, golang/node.js-based projects typically/historically involving collaboration among a majority of developers on non-Windows platforms - perhaps for reasons including the more subjective, but still relevant - aren't reasonably expected to show any sudden/additional "boon" to the agile, expedited release of higher quality software/features when shifting exclusively to a Windows-native development workflow, etc...). By extension, where organizational initiatives may seek to further process-engineer the delivery of better/faster products/services, and aligning these with objectives related to digital transformation - it would seem that even a cursory "bottom-up" examination of development workflows would starkly reveal this "irritant" as primarily responsible for continuously "chafing" any reasonable expectation for improved development efficiency.
